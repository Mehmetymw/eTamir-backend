    ...
    public class FavService : IFavService
    {
        ...
        public async Task<Response<FavsDto>> GetAll(string userId)
        {
            if (string.IsNullOrEmpty(userId))
                return Response<FavsDto>.Fail("UserId is null or empty", 400);

            var redis = _redisService.GetDatabase();
            var favs = await redis.HashGetAllAsync(userId);

            if (favs == null || favs.Length == 0)
                return Response<FavsDto>.Fail("User has no favs", 404);

            var favsDto = new FavsDto
            {
                UserId = userId,
                FavItems = favs.Select(fav => JsonConvert.DeserializeObject<FavItemDto>(fav.Value)).ToArray()
            };

            return Response<FavsDto>.Success(200, favsDto);
        }
        public async Task<Response<FavDto>> Get(FavDto favDto)
        {
            if (favDto is null)
                return Response<FavDto>.Fail("FavDto is null", 400);
